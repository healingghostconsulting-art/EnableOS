...
describe("ContentStore seam", () => {
  ...
  it("authorQuizContent edits a real module's apply checkpoint and stays tenant-scoped", () => {
    ...
    const firstId = before.modules[0]?.questions[0]?.id;
    ...
    expect(after.modules[0]?.questions.find((q) => q.id === firstId)?.prompt).toBe("Edited prompt for tenant-z");
    ...
    expect(other.modules[0]?.questions.find((q) => q.id === firstId)?.prompt).not.toBe("Edited prompt for tenant-z");
  });
});
